...
## Step 0. k8s/ 폴더 직접 만들기

CI가 이미지 태그를 교체하려면 `k8s/*.yaml` 파일이 먼저 존재해야 합니다.
지금 레포에는 `k8s/` 폴더가 없습니다 — 직접 만들겠습니다.
...
```bash title="터미널"
cat > k8s/order-api-deployment.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: order-api
  namespace: hanbat-<본인_GitHub_사용자명>
  labels:
    app: order-api
spec:
  replicas: 1
...
### k8s/order-api-service.yaml

```bash title="터미널"
cat > k8s/order-api-service.yaml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: order-api
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: payment-api
          image: <ACR_NAME>.azurecr.io/payment-api:PLACEHOLDER
          ports:
            - containerPort: 8080
          env:
            - name: FAULT_DELAY_MS
...
### k8s/payment-api-service.yaml

```bash title="터미널"
cat > k8s/payment-api-service.yaml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: payment-api
...
```bash title="터미널"
cat > k8s/order-web-deployment.yaml << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: order-web
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: order-web
          image: <ACR_NAME>.azurecr.io/order-web:PLACEHOLDER
          ports:
            - containerPort: 80
          livenessProbe:
            httpGet:
...
### k8s/order-web-service.yaml

```bash title="터미널"
cat > k8s/order-web-service.yaml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: order-web
...
!!! note "image: `PLACEHOLDER` 가 의도적입니다"
    `image: <ACR_NAME>.azurecr.io/order-api:PLACEHOLDER`의 `PLACEHOLDER`는 잠시 후 CI가 자동으로 실제 커밋 SHA로 교체합니다. 지금은 그대로 두세요.
