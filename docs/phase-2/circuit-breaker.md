...
!!! note "Fallback 응답이란?"
    외부 서비스(payment-api)가 실패했을 때 에러를 그대로 사용자에게 내보내는 대신, **미리 준비해둔 대체 응답**을 돌려주는 것입니다. 결제 정보는 못 가져오더라도 주문 화면 자체는 살아있게 해주는 것이 목적입니다.
...
!!! note "circuitbreaker — Circuit Breaker 라이브러리"
    Retry에 tenacity를 쓴 것처럼, Circuit Breaker는 **circuitbreaker** 라이브러리를 씁니다.
    `requirements.txt`에 이미 `circuitbreaker==2.0.0`이 포함되어 있으니 별도 설치는 필요 없습니다.

    사용법은 tenacity와 비슷하게 **데코레이터** 방식입니다:

    ```python
    from circuitbreaker import circuit, CircuitBreakerError

    @circuit(
        failure_threshold=5,   # 몇 번 실패하면 Open으로 전환할지
        recovery_timeout=10,   # Open 후 몇 초 뒤 Half-Open으로 전환할지
    ...
    함수에 `@circuit`을 붙이면 CB가 알아서 실패를 카운트합니다.
...
### Step 1-1 — `circuitbreaker` import 추가
...
!!! note "왜 함수를 두 개로 나눴나요?"
    ...
    ① _fetch_payment 호출 (@circuit 데코레이터가 붙어있음)
    ...
    Retry 3회 모두 실패 → _fetch_payment_inner가 예외를 던짐
                        → CB(@circuit)가 실패 1회 카운트
    ```

    - `_fetch_payment_inner`: 실제 HTTP 호출 + Retry (내부, `@retry`)
    - `_fetch_payment`: CB 게이트 역할 (외부, `@circuit`)
