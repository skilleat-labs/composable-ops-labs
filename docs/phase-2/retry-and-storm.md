...
!!! note "tenacity, circuitbreaker는 이미 설치되어 있습니다"
    `requirements.txt`를 보면 `tenacity==8.3.0`, `circuitbreaker==2.0.0`이 이미 있습니다.
    라이브러리 추가 없이 바로 코드 수정으로 넘어갑니다.

    **tenacity**는 Python에서 Retry 로직을 쉽게 붙이는 라이브러리입니다.
    Retry를 직접 구현하면 `for i in range(3): try/except` 같은 반복 코드가 필요한데,
    tenacity는 이걸 데코레이터 한 줄로 처리합니다:

    ```python
    @retry(stop=stop_after_attempt(3))   # 3번 시도
    async def call():
        ...
    ```

    몇 번 시도할지(`stop`), 얼마나 기다렸다 재시도할지(`wait`),
    어떤 예외가 났을 때 재시도할지(`retry`) — 이 세 가지를 조합해서 씁니다.

    **circuitbreaker**는 Circuit Breaker를 구현하는 라이브러리로, Phase 2-2에서 사용합니다.
