...
```bash title="터미널"
kubectl top pod pod-besteffort pod-burstable pod-guaranteed
```
...
Resource         Used    Hard
--------         ----    ----
count/pods       3       5        ← 현재 3개 Pod, 최대 5개
limits.cpu       700m    1        ← 현재 700m 사용
limits.memory    768Mi   512Mi    ← 이미 초과! (기존 Pod들 때문)
requests.cpu     430m    500m
requests.memory  394Mi   256Mi    ← 이미 초과!
```

!!! note "기존 Pod들이 이미 Quota를 채웠을 수 있습니다"
    앞 Step에서 만든 Pod들이 자원을 쓰고 있습니다.
    새 Pod 추가 시 Quota 초과가 발생하는 것을 확인합니다.
...
```bash title="터미널"
kubectl delete pod pod-besteffort pod-burstable pod-guaranteed no-resources --ignore-not-found
kubectl delete pod pod-quota-exceed pod-over-max --ignore-not-found
